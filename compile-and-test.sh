...
INFLUXDB_VERSIONS="1.4"
JAVA_VERSIONS="3-jdk-8-alpine"
...
if [ "$USE_PROXY" == "nginx" ] ; then
    ...
    INFLUXDB_API_URL=http://nginx:80/influx-api
fi
...
do
 ...
 if [ "$USE_PROXY" == "nginx" ] ; then
     ...
     docker run \
            ...
            --name nginx \
            --publish 8888:80 \
            --volume ${BUILD_HOME}/src/test/nginx/nginx.conf:/etc/nginx/nginx.conf:ro \
