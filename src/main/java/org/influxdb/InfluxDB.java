 ...
 *
 * <p>A full list of currently available interfaces is implemented in:
 *
 * <p><a
...
public interface InfluxDB extends AutoCloseable {
  ...
  public enum LogLevel {
     ...
     *
     * <p>Note: This requires that the entire request and response body be buffered in memory!
     */
    FULL;
    /**
     * Parses the string argument as a LogLevel constant.
     *
     * @param value a {@code String} containing the {@code LogLevel constant} representation to be
     *     parsed
     * @return the LogLevel constant representation of the param or {@code NONE} for null or any
  ...
  /**
   * Enable batching of single Point writes to speed up writes significantly. This is the same as
   * calling InfluxDB.enableBatch(BatchOptions.DEFAULTS)
   *
   * @return the InfluxDB instance to be able to use it in a fluent manner.
   */
  ...
  /**
   * Enable batching of single Point writes to speed up writes significantly. If either number of
   * points written or flushDuration time limit is reached, a batch write is issued. Note that batch
   * processing needs to be explicitly stopped before the application is shutdown. To do so call
   * disableBatch().
   *
  ...
  /**
   * Enable batching of single Point writes as {@link #enableBatch(int, int, TimeUnit,
   * ThreadFactory)}} using {@linkplain java.util.concurrent.Executors#defaultThreadFactory()
  ...
  /**
   * Enable batching of single Point writes as {@link #enableBatch(int, int, TimeUnit,
   * ThreadFactory, BiConsumer)} using with a exceptionHandler that does nothing.
   *
   * @param actions the number of actions to collect
   * @param flushDuration the time to wait at most.
   * @param flushDurationTimeUnit the TimeUnit for the given flushDuration.
  ...
  /**
   * Enable batching of single Point writes with consistency set for an entire batch flushDurations
   * is reached first, a batch write is issued. Note that batch processing needs to be explicitly
   * stopped before the application is shutdown. To do so call disableBatch(). Default consistency
   * is ONE.
   *
   * @param actions the number of actions to collect
   * @param flushDuration the time to wait at most.
   ...
   * flushDurations is reached first, a batch write is issued. Note that batch processing needs to
   * be explicitly stopped before the application is shutdown. To do so call disableBatch().
   *
   * @param actions the number of actions to collect
   * @param flushDuration the time to wait at most.
   * @param flushDurationTimeUnit the TimeUnit for the given flushDuration.
   ...
   * @param batchPoints the points to write in BatchPoints.
   */
   ...
   * Write a set of Points to the influxdb database with the new (&gt;= 0.9.0rc32) lineprotocol.
   *
   * <p>If batching is enabled with appropriate {@code BatchOptions} settings ({@code
   * BatchOptions.bufferLimit} greater than {@code BatchOptions.actions}) This method will try to
   * retry in case of some recoverable errors. Otherwise it just works as {@link
   * #write(BatchPoints)}
   *
   * @see <a href="https://github.com/influxdb/influxdb/pull/2696">2696</a>
   * @see <a
   *     href="https://github.com/influxdata/influxdb-java/wiki/Handling-errors-of-InfluxDB-under-high-load">
   ...
   * Execute a query against a database.
   *
   * <p>One of the consumers will be executed.
   *
   * @param query the query to execute.
   ...
   */
  public void query(
      Query query, int chunkSize, BiConsumer<Cancellable, QueryResult> onNext, Runnable onComplete);
   ...
   * @deprecated (since 2.9, removed in 3.0) Use <code>org.influxdb.InfluxDB.query(Query)</code> to
   *     execute a parameterized <strong>CREATE DATABASE</strong> query.
   */
  @Deprecated
   ...
   * @deprecated (since 2.9, removed in 3.0) Use <code>org.influxdb.InfluxDB.query(Query)</code> to
   *     execute a <strong>DROP DATABASE</strong> query.
   */
  @Deprecated
   ...
   * @deprecated (since 2.9, removed in 3.0) Use <code>org.influxdb.InfluxDB.query(Query)</code> to
   *     execute a <strong>SHOW DATABASES</strong> query.
   */
  @Deprecated
   ...
   * @deprecated (since 2.9, removed in 3.0) Use <code>org.influxdb.InfluxDB.query(Query)</code> to
   *     execute a <strong>SHOW DATABASES</strong> query and inspect the result.
   */
  @Deprecated
  public boolean databaseExists(final String name);

  /**
   * Send any buffered points to InfluxDB. This method is synchronous and will block while all
   * pending points are written.
   *
   ...
   * @deprecated (since 2.9, removed in 3.0) Use <code>org.influxdb.InfluxDB.query(Query)</code> to
   *     execute a parameterized <strong>CREATE RETENTION POLICY</strong> query.
   */
  @Deprecated
   ...
   * @param database the name of the database
   * @deprecated (since 2.9, removed in 3.0) Use <code>org.influxdb.InfluxDB.query(Query)</code> to
   *     execute a <strong>DROP RETENTION POLICY</strong> query.
   */
  @Deprecated
