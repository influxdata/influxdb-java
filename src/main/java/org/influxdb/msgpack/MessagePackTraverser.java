...
public class MessagePackTraverser {
  ...
  /**
   * Traverse over the whole message pack stream. This method can be used for converting query
   * results in chunk.
   *
  ...
  /**
   * Parse the message pack stream. This method can be used for converting query result from normal
   * query response where exactly one QueryResult returned
   *
   * @param is The MessagePack format input stream
   * @return QueryResult
   */
  ...
  void traverse(
      ...
      final QueryResultModelPath queryResultPath,
      final int readAmount)
      throws IOException {
    int amount = 0;
      ...
      switch (type) {
        ...
        case ARRAY:
          ...
          if (length > 0) {
            if ("results".equals(lastStringNode)) {
              ...
              addedName = "results";
              addedObject = results;
            } else if ("series".equals(lastStringNode)
                && queryResultPath.compareEndingPath("result")) {
              Result result = queryResultPath.getLastObject();
              List<Series> series = new ArrayList<>();
              result.setSeries(series);
              addedName = "seriesList";
              addedObject = series;
            } else if ("columns".equals(lastStringNode)
                && queryResultPath.compareEndingPath("series")) {
              Series series = queryResultPath.getLastObject();
              List<String> columns = new ArrayList<>();
              series.setColumns(columns);
              addedName = "columns";
              addedObject = columns;
            } else if ("values".equals(lastStringNode)
                && queryResultPath.compareEndingPath("series")) {
              Series series = queryResultPath.getLastObject();
        ...
        case EXTENSION:
          ...
          if (extension.getType() == MSG_PACK_TIME_EXT_TYPE) {
            // decode epoch nanos in accordance with
