...
public enum TimeUtil {
  ...
  /**
   * convert a unix epoch time to timestamp used by influxdb. this can then be used in query
   * expressions against influxdb's time column like so: influxDB.query(new Query("SELECT * FROM
   * some_measurement WHERE time &gt;= '" + toInfluxDBTimeFormat(timeStart) + "'", some_database))
  ...
  /**
   * convert an influxdb timestamp used by influxdb to unix epoch time. influxdb time format
   * example: 2016-10-31T06:52:20.020Z or 2016-10-31T06:52:20Z
   *
