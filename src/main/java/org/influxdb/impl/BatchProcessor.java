...
public final class BatchProcessor {
  ...
  public static final class Builder {
    ...
    public Builder interval(
        final int flushInterval, final int jitterInterval, final TimeUnit unit) {
    ...
    /**
     * A buffer for failed writes so that the writes will be retried later on. When the buffer is
     * full and new points are written, oldest entries in the buffer are lost.
     *
    ...
    /**
     * To define the behaviour when the action queue exhausts. If unspecified, will default to false
     * which means that the {@link InfluxDB#write(Point)} will be blocked till the space in the
     * queue is created. true means that the newer actions being written to the queue will dropped
     * and {@link BatchProcessor#droppedActionHandler} will be called.
     *
     * @param dropActionsOnQueueExhaustion the dropActionsOnQueueExhaustion
     * @return this Builder to use it fluent
     */
    ...
    public BatchProcessor build() {
      ...
      if (this.bufferLimit > this.actions) {
        batchWriter =
            new RetryCapableBatchWriter(
                this.influxDB, this.exceptionHandler, this.bufferLimit, this.actions);
      } else {
      ...
      return new BatchProcessor(
          ...
          this.precision,
          this.dropActionsOnQueueExhaustion,
    ...
    Runnable flushRunnable =
        new Runnable() {
          ...
          public void run() {
            ...
            BatchProcessor.this.scheduler.schedule(
                this,
                BatchProcessor.this.flushInterval + jitterInterval,
                BatchProcessor.this.flushIntervalUnit);
          }
        };
  ...
  /**
   * Flush the current open writes to InfluxDB. This will block until all pending points are
   * written.
   */
  void flush() {
    this.write();
